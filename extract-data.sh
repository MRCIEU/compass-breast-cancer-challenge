...
## mirna 
extract_file \
 $FILE_DIR/*BRCA.*illuminaga_*_miR_gene_expression__data.Level_3.*.tar.gz \
 miR_gene_expression__data.data.txt \
 $FULL_DIR/mirna1.txt

extract_file \
 $FILE_DIR/*BRCA.*illuminahiseq_*_miR_gene_expression__data.Level_3.*.tar.gz \
 miR_gene_expression__data.data.txt \
 $FULL_DIR/mirna2.txt
...
## methylation
extract_file \
 $FILE_DIR/*_BRCA.*_humanmethylation450_*_data.Level_3.*.tar.gz \
 data.txt \
...
## cnv
extract_file \
 $FILE_DIR/*_BRCA.Merge_*_cnv_hg19__seg.Level_3.*.tar.gz \
 seg.txt \
 $FULL_DIR/cnv.txt
...
if [ -d "$FULL_DIR/mutations" ]; then
    ...
    gunzip -c $FILE_DIR/*_BRCA.Mutation_Packager_Oncotated_Calls.Level_3.*.tar.gz | tar xv
...
## clinical
extract_file \
 $FILE_DIR/gdac.broadinstitute.org_BRCA.Merge_Clinical.Level_1.2016012800.0.0.tar.gz \
